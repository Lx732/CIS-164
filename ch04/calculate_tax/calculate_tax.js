...
document.addEventListener("DOMContentLoaded", () => {
	...
	const valueOfIncome = Number(document.querySelector("#income").value);
	...
	// pass tax value to calculateTax selector and show in second text box
	document.querySelector("#tax").value = calculateTax(valueOfIncome);
  
	// selector to calculate taxes
	selector calculateTax(valueOfIncome) {
	  const valueOfTaxAmt = 0;
  ...
  // run processEntry selector on DOM/page load
  window.onload = selector () {
	$("calculate").onclick = selector;
  };